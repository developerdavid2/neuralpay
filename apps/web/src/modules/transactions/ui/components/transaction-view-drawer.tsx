...
export function TransactionViewDrawer() {
  ...
  const receiptId = tx ? `#TRX-${tx.id.slice(-8).toUpperCase()}` : null;
  ...
  return (
    <>
      ...
      >
        ...
        >
          {isLoading || !tx ? (
            <TransactionViewDrawerSkeleton onClose={onClose} />
          ) : (
            <>
              <DrawerHeader className="px-6 py-4 border-b space-y-4 shrink-0">
                {/* Top row: receipt ID + edit icon + close */}
              ...
              {/* Footer: 2 buttons only, px-6 py-4 to match header */}
              <DrawerFooter className="px-6 py-4 border-t space-y-2 shrink-0">
                ...
                {/* Delete — available for any non-synced transaction */}
                {!isSynced && (
                  <Button
                    variant="outline"
...
// Skeleton that mimics the exact header + footer structure
function TransactionViewDrawerSkeleton({ onClose }: { onClose: () => void }) {
  return (
    <>
      {/* Header skeleton */}
