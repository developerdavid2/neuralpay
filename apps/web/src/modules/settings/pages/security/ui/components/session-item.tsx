    ...
    <div className="flex items-center justify-between gap-4 p-4 border-border bg-main-tint rounded-xl">
      <div className="flex items-center gap-3 max-w-75">
        <div className="relative shrink-0">
          ...
          <PlatformIcon className="absolute -bottom-1 -right-1 size-5 rounded-full bg-background p-px text-muted-foreground" />
        </div>
      ...
      {session.isCurrent ? (
        ...
        >
          {isRevoking ? <Spinner className="size-4 " /> : "Sign Out"}
        </Button>
      )}
    </div>
  );
