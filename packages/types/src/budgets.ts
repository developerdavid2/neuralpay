...
export type BudgetSortField = "date" | "spent" | "limitAmount" | "name";
...
const updateBudgetBaseSchema = z.object({
  id: z.uuid(),
...
export const updateBudgetSchema = updateBudgetBaseSchema
  .refine(
    (data) => {
...
// Form-facing variant: id omitted at the base (pre-refinement) level, then
// the same two refinements re-applied so client validation still enforces
// them. Kept in sync with updateBudgetSchema above — if you add a refine
// there, mirror it here.
export const updateBudgetFormSchema = updateBudgetBaseSchema
  .omit({ id: true })
  .refine(
    (data) => {
      if (!data.categories || data.limitAmount === undefined) return true;
...
export type UpdateBudgetFormInput = z.input<typeof updateBudgetFormSchema>;
