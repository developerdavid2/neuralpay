...
```bash
bun -F @neuralpay/ui check-types      # tsc --noEmit
```

## Conventions

- Tokens live in `src/styles/globals.css` using the `@theme` block with `--font-nexa` and `--font-rostex`
- Exported as `@neuralpay/ui/globals.css`, `/components/*`, `/lib/*`, `/hooks/*`
- `cn()` helper in `src/lib/utils.ts`

## Gotchas

- The web app imports the global CSS on one line: `@import "@neuralpay/ui/globals.css"`
